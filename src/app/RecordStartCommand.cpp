 ...
 *
 *   - Creates the logger routes directly on the MMS+.
 ...
 *
 * Logger IDs printed by this command are diagnostics only.  A later sync
 * reconstructs the active logger routes directly from the board using
    ...
    namespace {
        ...
        void configureGyro(
            ...
            const MblMwGyroBoschOdr odr =
                gyroOdrFromRate(sample_rate_hz);
        ...
        void onAccelLoggerCreated(
            ...
            auto* state =
                static_cast<LoggerCreateState*>(context);

            if (logger == nullptr) {
                std::cout
                    << "Accel logger creation failed\n";

                state->failures++;
            }
            else {
                std::cout
                    << "Accel logger created\n";
        ...
        void onBatteryTimerCreated(
            ...
            auto* state =
                static_cast<TimerCreateState*>(context);
            ...
            if (timer == nullptr) {
                std::cout
                    << "Battery timer creation failed\n";

                state->failed.store(true);
            }
            else {
                std::cout
                    << "Battery timer created\n";
        ...
        void onBatteryTimerCommandsRecorded(
            ...
            auto* state =
                static_cast<EventRecordState*>(context);
            ...
            if (status != 0) {
                std::cout
                    << "Battery timer command recording failed, "
                    << "status="
                    << status
                    << "\n";
                ...
                std::cout
                    << "Battery timer command recording complete\n";
            }
        ...
        bool tryCreateTimerIndefinite(
            ...
            std::cout
                << "  trying mbl_mw_timer_create_indefinite "
                << "period="
                << period_ms
                << " ms\n";
        ...
        bool tryCreateTimerLegacy(
            ...
            std::cout
                << "  trying mbl_mw_timer_create "
                << "repetitions=0 period="
                << period_ms
                << " ms\n";
        ...
        void runTimerDiagnostics(
            ...
            for (const std::uint32_t period_ms : periods) {
                if (period_ms == 0) {
                    continue;
                }

                std::cout
                    << "Probing timer period: "
                    << period_ms
                    << " ms\n";
                ...
                if (
                    ...
                    const std::uint8_t timer_id =
                        mbl_mw_timer_get_id(timer);

                    std::cout
                        << "  SUCCESS indefinite timer, id="
                        << static_cast<int>(timer_id)
                        << "\n";

                    std::cout
                        << "  removing diagnostic timer\n";
                ...
                std::cout
                    << "  FAILED both timer APIs for period "
                    << period_ms
                    << " ms\n";
            }

            std::cout
                << "End timer diagnostics.\n\n";
        }
    ...
    int runRecordStartCommand(
        ...
        if (battery_enabled) {
            ...
            if (
                ...
                std::cerr
                    << "Battery logging cannot continue because "
                    << "no board timer could be allocated.\n";
        ...
        if (
            ...
            std::cerr
                ...
                << "accelerometer logger creation\n";

            return 4;
        }

        std::cout
            << "Creating gyro logger\n";
        ...
        if (logger_state.failures.load() != 0) {
            std::cerr
                << "One or more logger creations failed\n"
                << "The board probably still has old "
                << "logger routes allocated.\n"
                << "Run record-reset before starting "
                << "a fresh session.\n";
        ...
        if (
            battery_enabled &&
            logger_state.battery_logger == nullptr
            ) {
            std::cerr
                << "Battery logger is null "
                << "after successful callback\n";
        ...
        /*
         * These IDs are useful while diagnosing board allocation,
         * but they are deliberately not persisted.  sync reconstructs
         * logger routes from the physical board instead.
         */
        ...
        if (battery_enabled) {
            std::cout
                << "Recording battery read command "
                << "onto timer\n";
            ...
            if (
                ...
                std::cerr
                    << "Timed out waiting for battery "
                    << "timer command recording\n";
        ...
        /*
         * From this point onward the recording is entirely board-resident.
         * No host-side timestamp, logger metadata, or serialized SDK state
         * is created.
         */
        std::cout
            << "Starting internal logging, overwrite=false\n";
        ...
        std::cout
            << "Starting accelerometer sampling\n";
        ...
        if (battery_enabled) {
            std::cout
                << "Battery logging enabled: every "
                << battery_interval_seconds
                << " seconds\n";
        }
        else {
            std::cout
                << "Battery logging disabled\n";
        }

        std::cout
            << "The MMS+ is now internally logging "
            << "accel + gyro";
        ...
        std::cout
            << ".\n"
            << "Use record-stop before sync/download.\n";
